...
// include: C:\Users\THECU~1.BIG\AppData\Local\Temp\tmpgomxdeta.js
...
// end include: C:\Users\THECU~1.BIG\AppData\Local\Temp\tmpgomxdeta.js
// include: C:\Users\THECU~1.BIG\AppData\Local\Temp\tmp_kx9xoyc.js

    // All the pre-js content up to here must remain later on, we need to run
    // it.
    if ((typeof ENVIRONMENT_IS_WASM_WORKER != 'undefined' && ENVIRONMENT_IS_WASM_WORKER) || (typeof ENVIRONMENT_IS_PTHREAD != 'undefined' && ENVIRONMENT_IS_PTHREAD) || (typeof ENVIRONMENT_IS_AUDIO_WORKLET != 'undefined' && ENVIRONMENT_IS_AUDIO_WORKLET)) Module['preRun'] = [];
    var necessaryPreJSTasks = Module['preRun'].slice();
  // end include: C:\Users\THECU~1.BIG\AppData\Local\Temp\tmp_kx9xoyc.js
// include: C:\Users\THECU~1.BIG\AppData\Local\Temp\tmps75in10u.js

    if (!Module['preRun']) throw 'Module.preRun should exist because file support used it; did a pre-js delete it?';
    necessaryPreJSTasks.forEach((task) => {
      if (Module['preRun'].indexOf(task) < 0) throw 'All preRun tasks that exist before user pre-js code should remain after; did you replace Module or modify Module.preRun?';
    });
  // end include: C:\Users\THECU~1.BIG\AppData\Local\Temp\tmps75in10u.js
...
var ASM_CONSTS = {
  142496: () => { if (document.fullscreenElement) return 1; },  
 142542: () => { return document.getElementById('canvas').width; },  
 142594: () => { return parseInt(document.getElementById('canvas').style.width); },  
 142662: () => { document.exitFullscreen(); },  
 142689: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 142762: () => { if (document.fullscreenElement) return 1; },  
 142808: () => { return document.getElementById('canvas').width; },  
 142860: () => { return screen.width; },  
 142885: () => { document.exitFullscreen(); },  
 142912: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 143045: () => { return window.innerWidth; },  
 143071: () => { return window.innerHeight; },  
 143098: () => { if (document.fullscreenElement) return 1; },  
 143144: () => { return document.getElementById('canvas').width; },  
 143196: () => { return parseInt(document.getElementById('canvas').style.width); },  
 143264: () => { if (document.fullscreenElement) return 1; },  
 143310: () => { return document.getElementById('canvas').width; },  
 143362: () => { return screen.width; },  
 143387: () => { return window.innerWidth; },  
 143413: () => { return window.innerHeight; },  
 143440: () => { if (document.fullscreenElement) return 1; },  
 143486: () => { return document.getElementById('canvas').width; },  
 143538: () => { return screen.width; },  
 143563: () => { document.exitFullscreen(); },  
 143590: () => { if (document.fullscreenElement) return 1; },  
 143636: () => { return document.getElementById('canvas').width; },  
 143688: () => { return parseInt(document.getElementById('canvas').style.width); },  
 143756: () => { document.exitFullscreen(); },  
 143783: ($0) => { document.getElementById('canvas').style.opacity = $0; },  
 143841: () => { return screen.width; },  
 143866: () => { return screen.height; },  
 143892: () => { return window.screenX; },  
 143919: () => { return window.screenY; },  
 143946: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 143999: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 144070: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 144127: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 144383: ($0) => { document.getElementById('canvas').style.cursor = UTF8ToString($0); },  
 144454: () => { if (document.fullscreenElement) return 1; },  
 144500: () => { return window.innerWidth; },  
 144526: () => { return window.innerHeight; },  
 144553: () => { if (document.pointerLockElement) return 1; },  
 144600: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === window.miniaudio.device_state.started) { device.webaudio.resume().then(() => { Module._ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 146758: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 146922: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 147026: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 ...
 150025: ($0) => { return miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 150091: ($0) => { var device = miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 150484: ($0) => { miniaudio.untrack_device_by_index($0); },  
 150527: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 150652: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; }
};
