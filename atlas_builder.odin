...
load_png_tileset :: proc(filename: string) -> (Tileset, bool) {
	data, data_ok := os.read_entire_file(filename, context.allocator)
...
main :: proc() {
	...
	if font_data, ok := os.read_entire_file(FONT_FILENAME, context.allocator); ok == nil {
		fi: stbtt.fontinfo
	...
	img_write :: proc "c" (ctx: rawptr, data: rawptr, size: c.int) {
        ...
        err := os.write_entire_file(ATLAS_PNG_OUTPUT_PATH, slice.bytes_from_ptr(data, int(size)))
	    assert(err == nil, "Error during image write!")
    }
